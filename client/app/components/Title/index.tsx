        ...
        <div className={style.boxWrapper}>
            <div className={style.boxTitle}>
                <h1 className={style.title}>
                    Leading global partner in <span>
                        {TITLE_CHANGING_WORDS.map((word, wordIndex) => (
                            <b className={`${style.changingText} ${index === wordIndex ? style.isVisible : style.isHidden}`} key={wordIndex}>
                                {word}
                            </b>
                        ))}
                    </span>
                    ...
                    committed to delivering <span>
                        {TITLE_SECOND_CHANGING_WORDS.map((word, wordIndex) => (
                            <b className={`${style.secondChangingText} ${secondIndex === wordIndex ? style.isVisible : style.isHidden}`} key={wordIndex}>
                                {word}
                            </b>
                        ))}
                    </span>
